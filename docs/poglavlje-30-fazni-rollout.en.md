...
![Timeline of the phased rollout: a numbered plan, reality exposing a gap the alert didn't catch, two inserted steps, a reordered sequence, and a replaced alerting mechanism, with the most critical part of the fleet deliberately placed last.](diagrams/ch30-vremenska-linija.en.png){: width="95%" }

![Audit before shutting down the old alerting system: of 23 old alerts, 16 hadn't received a single data point in over a year — they looked "green" only because the absence of data had been interpreted as a normal state.](diagrams/dashboard-alarm-audit.en.png){: width="95%" }
...
![A skipped step with a written condition for its return: the estimated worst case is named with a number, and the plan automatically reverts to the step the moment the pilot shows measurable loss.](diagrams/ch30-preskocen-korak.en.png){: width="80%" }
